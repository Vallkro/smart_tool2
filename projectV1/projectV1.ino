...
void setup()
{
  ...
  previousMillis = millis();
    ...
    myShort.number=699;
    myShort2.number=713;
    dataToSend[0]=myShort.bytes[0];
    dataToSend[1]=myShort.bytes[1];
    dataToSend[2]=myShort2.bytes[0];
    dataToSend[3]=myShort2.bytes[1];
}
...
void sequenceA() {
  ...
  if (state == "executing" && stage == 1 && (angle.number >= 3000 || (currentSequenceMillis - prevSequenceMillis) >= 1000)) {
